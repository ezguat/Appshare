...
if(isset($_GET['port']))
{
    ...
    $port = "21";//我们要检测制定端口号
    $port443 = "443";//我们要检测制定端口号
    $port22 = "22";//我们要检测制定端口号
    $port23 = "23";//我们要检测制定端口号
    $timeout = "2";//这个地方是超时时间，即2秒钟，可以随意设定
    if($server and $port and $timeout and $port443 and $port22){
        $verbinding = @fsockopen("$server", $port, $errno, $errstr, $timeout);
        $verbinding443 = @fsockopen("$server", $port443, $errno, $errstr, $timeout);
        $verbinding22 = @fsockopen("$server", $port443, $errno, $errstr, $timeout);
        $verbinding23 = @fsockopen("$server", $port443, $errno, $errstr, $timeout);
    }
    if($verbinding) {
