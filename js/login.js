        ...
        if($('#account').val()&&$('#password').val()){
            $.get("../php/login.php",{name:$('#account').val(),password:$('#password').val()},function (data) {
                if(data==false)
                {
                    ...
                    var time=new Date()
                    $.get("../php/login.php",{system:systemversion,browser:browser,year:time.getFullYear(),month:time.getMonth()+1,day:time.getDate(),log_time:time.toTimeString().split('GMT')[0],local_ip:restoredSession},function(data) {
                  		alert(data)
                  		  });
